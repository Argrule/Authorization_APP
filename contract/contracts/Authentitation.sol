...
contract Authentitation {
    ...
    function register(string memory _name, string memory _password) public {
        //注册函数
        require(
            !addressToUser[msg.sender].hasRegistered,
            "User is already registered."
        );
        // if (addressToUser[msg.sender].hasRegistered) {
    ...
    function login(string memory _name, string memory _password) public {
        ...
        //用户名、密码全部匹配成功才能成功登录
        require(
            keccak256(abi.encodePacked(addressToUser[msg.sender].name)) ==
                keccak256(abi.encodePacked(_name)) &&
                addressToUser[msg.sender].password ==
                sha256(abi.encodePacked(_password))
        );
    ...
    function modify(string memory _newName, string memory _newPassword) public {
        ...
        addressToUser[msg.sender].password = sha256(
    ...
    function logout() public {
        //注销账号
        require(
            addressToUser[msg.sender].hasLogin &&
                addressToUser[msg.sender].hasRegistered
        ); //已注册且已登录
